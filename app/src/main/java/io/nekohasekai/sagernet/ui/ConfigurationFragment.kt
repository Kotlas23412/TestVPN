...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_subscription_auto_https_cleanup -> runSubscriptionAutoCleanupFlow()
            R.id.action_subscription_manual_test_cleanup -> runSubscriptionManualCleanupFlow()
    ...
    @OptIn(DelicateCoroutinesApi::class)
    fun runSubscriptionAutoHttpsCleanupSingle() {
        if (DataStore.runningTest) {
            snackbar("Тестирование уже запущено! Дождитесь окончания.").show()
            return
        }
